...
void eval(char *cmdline) 
{
   ...
   /* Holds modified command line */
	...
	int state;
	int jid=nextjid;// next few lines of code to add job
	if(bg==1)
		state=BG;
	else
		state=FG;
	//printf("pid before add  %d",(int) getpid());
	addjob(jobs, pid, state, cmdline);
	//printf("job added pid = %d",(int)getpid());
	...
	if (!bg) {
		...
		printf("[%d] (%d) %s",jid, pid, cmdline);// TODO print the jid of the process
	}
...
int builtin_cmd(char **argv) 
{
	...
	exit(0);
		...
		//printf("%s",argv[0]);
		if(!strcmp(argv[0],"jobs"))// lists the background jobs
		{
			int i;
			//printf("in jobs");
			...
			for (i = 0; i < MAXJOBS; i++) {
				...
				return 1;
		}// end of printing jobs
			else if(!strcmp(argv[0],"fg")||!strcmp(argv[0],"bg")) // calls the function bgfg
			{	do_bgfg(argv);
				return 1;
			}
...
//		printf("jid is %d",jid);
	...
	if(!strcmp(argv[0],"fg"))  // for fg TODO it is far from perfect.... question not understood properly
	{
	...
	{
		if(jobs[job_id].state == ST)
		{
			kill(pid,SIGCONT);
			jobs[job_id].state=BG;
			printf("[%d] (%d) %s",jid, pid, jobs[job_id].cmdline);
		}
	}
	return;
}
...
void sigint_handler(int sig) 
{
	...
	for(i=0;i<MAXJOBS;i++)
	{
		if(jobs[i].state==FG)
		{	
			//printf("pid killed i %d", (int) jobs[i].pid);
